...
export default {
    ...
    computed: {
        // 过滤 L2 单品为 KA Fixed
        kaFixedSkus() { return this.items.filter(i => i.product_code === 'KA Fixed'); },
        ...
        filteredPlans() {
            return this.plans.filter(p => {
                ...
                // 如果是无限额，进度给0只是为了防止报错，UI上会展示 --
                progress: (p.is_unlimited_quota || !p.total_issuance_amount) ? 0 : Math.min(100, Math.floor((p.sold_amount / p.total_issuance_amount) * 100))
            }));
        }
    },
    methods: {
        ...
        getBusinessStatus(p) {
            ...
            // 如果不是无限额，且已售满，则为已售罄
            if (!p.is_unlimited_quota && p.sold_amount >= p.total_issuance_amount) return '已售罄';
        ...
        initCreate() {
            ...
            this.editingPlan = { 
                plan_id: 'PLAN_KAFX_' + Date.now(), 
                item_code: 'KA_Fixed_Base', 
                name: 'KA VIP Promo New', 
                alias: 'VIP Exclusive',
                is_unlimited_quota: false, // KA 专属属性
                total_issuance_amount: 500000000, sold_amount: 0, min_sub_amount: 100000, max_single_sub: 10000000, cum_sub_limit: 50000000,
                period_days: 30, status: 'Draft', 
                sale_start_time: this.systemDate, 
                sale_end_time: nextYearStr,
                rates: [{ stepMinAmount: 0, rate: 0.18, displayRate: 18 }],
                benchmark_rate: 0.12
            };
            this.showModal = true;
        },
        ...
        handleSave() {
            ...
            // 如果开启了无限额模式，清空限额字段
            if (this.editingPlan.is_unlimited_quota) {
                this.editingPlan.total_issuance_amount = null;
                this.editingPlan.max_single_sub = null;
                this.editingPlan.cum_sub_limit = null;
            }

            const isKaFixed = true; // 标记这是 KA Fixed 的操作
            if (this.modalMode === 'create') {
                this.$emit('initiate-approval', this.editingPlan, 'save_draft', isKaFixed); 
            } else {
                const actionType = this.editingPlan.status === 'Draft' ? 'save_draft' : 'modify_fixed';
                this.$emit('initiate-approval', this.editingPlan, actionType, isKaFixed);
            }
            this.showModal = false;
        },
        applyListing(plan) {
            const planCopy = JSON.parse(JSON.stringify(plan));
            this.$emit('initiate-approval', planCopy, 'apply_listing', true);
        },
        applyOffShelf(plan) {
            if (!confirm('确定要申请下架该单品吗？')) return;
            const planCopy = JSON.parse(JSON.stringify(plan));
            this.$emit('initiate-approval', planCopy, 'off_shelf', true);
        },
        handleDeletePlan(plan) {
            if (!confirm('确定要删除该草稿吗？此操作不可恢复。')) return;
            this.$emit('delete-plan', plan, true); // true means isKaFixed
            this.showModal = false;
        },
        ...
        <div class="space-y-6">
            <div class="flex justify-between items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <!-- KA Fixed 权限点校验 -->
                    <button v-if="hasPermission('KA_FIXED_OPS:PLAN:CREATE')" @click="initCreate" class="bg-opay hover:bg-opay-hover text-white px-4 py-2 rounded shadow font-bold flex items-center transition">
            ...
            <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase w-[25%]">KA单品信息</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <tr v-for="plan in filteredPlans" :key="plan.plan_id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-3">
                                <!-- KA Fixed 特殊逻辑：无限额模式下进度展示 "--" -->
                                <div v-if="plan.is_unlimited_quota" class="text-gray-400 font-bold text-lg text-center">
                                    --
                                </div>
                                <div v-else>
                            ...
                            <td class="px-6 py-3 text-right">
                                <div class="flex flex-col items-end gap-1">
                                    ...
                                    <div v-else class="flex gap-2 justify-end">
                                        <!-- KA Fixed 权限控制 -->
                                        <button v-if="hasPermission('KA_FIXED_OPS:PLAN:EDIT') && !['已过期', '已下架'].includes(plan.displayStatus)" 
                                        ...
                                        <button v-if="plan.displayStatus === '草稿' && hasPermission('KA_FIXED_OPS:PLAN:PUBLISH')" 
                                        ...
                                        <button v-if="plan.displayStatus === '草稿' && hasPermission('KA_FIXED_OPS:PLAN:EDIT')" 
                                        ...
                                        <button v-if="['开放中', '已上架待开放', '已售罄'].includes(plan.displayStatus) && hasPermission('KA_FIXED_OPS:PLAN:OFFSHELF')" 
            ...
            <!-- 使用复用的表单组件，传入 isKaFixed=true 开启无限额开关 -->
            <FixedPlanForm v-if="showModal" v-model="editingPlan" :mode="modalMode" :fixed-items="kaFixedSkus" :isKaFixed="true" @close="showModal = false" @save="handleSave" @delete="handleDeletePlan" />
        </div>
    `
};